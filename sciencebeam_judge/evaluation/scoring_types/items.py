...
def _score_items_fuzzy_to(haystack, needles, threshold):
    return _score_items_to(
        haystack, needles, partial(
            _get_fuzzy_matched_characters, threshold=threshold
        )
    )
...
def get_score_items_to_fn_for_scoring_method(scoring_method):
    ...
    LOGGER.debug(
        'scoring method (%s) has no equivalent item matcher', scoring_method.name
    )
    return None
...
def _score_items(expected_items, actual_items, scoring_method):
    ...
    score_items_to_fn = get_score_items_to_fn_for_scoring_method(
        scoring_method
    )
